...
在 Obsidian 中打开 `INDEX.base` 来浏览、排序和过滤你的知识页面。当知识库增长后运行 `/wiki` 重新生成它。
...
这会添加 v2 agent 目录、`SCHEMA.md` 和 `log.md`，同时保留 v1 的所有内容。
...
默认库模板在 `AGENTS.md` 中还包含 **Knowledge Retrieval (Q&A)** 小节：用 `INDEX.base` 做发现，需要分类或布局规则时读 `SCHEMA.md`，再用 Obsidian CLI 的 `search` 与 `read`，并按优先级筛选（agent 目录、tags/domain、`status`、`updated`），最后用 wikilink 引用作答。**`/ask`** 技能是完整、权威的协议，避免多处重复导致漂移。这条链路把 **渐进式信息发现**（frontmatter 与链接）落实为可执行的、基于库的问答步骤。
...
5. **链式导航**：SCHEMA.md → INDEX.base → agent 页面 → 来源笔记 → 细节
